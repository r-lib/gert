...
Before you can do anything with Git, you must first configure your user name and email. You can do this with `git_config_global_set()`:

```{r eval = FALSE}
git_config_global_set("user.name", "Jane Doe")
git_config_global_set("user.email", "jane@example.com")
```

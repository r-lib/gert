...
TMP_DIR="${PWD}/libgit2/lib"
TMP_LIBS="-L${TMP_DIR} -lstatgit2 -lrt -lpthread -lstatssh2 -lstatssl -lstatcrypto -ldl -lpcre -lz"
else
...
TMP_DIR="${PWD}/libgit2-1.4.2-x86_64_linux/lib"
TMP_LIBS="-L${TMP_DIR} -lstatgit2 -lrt -lpthread -lstatssh2 -lstatssl -lstatcrypto -ldl"
fi
if "${R_HOME}/bin/R" -s -e "curl::curl_download('$URL','bundle.tar.gz')"; then
  tar xzf bundle.tar.gz && rm -f bundle.tar.gz
  cp $TMP_DIR/libgit2.a $TMP_DIR/libstatgit2.a
  cp $TMP_DIR/libssh2.a $TMP_DIR/libstatssh2.a
  cp $TMP_DIR/libssl.a $TMP_DIR/libstatssl.a
  cp $TMP_DIR/libcrypto.a $TMP_DIR/libstatcrypto.a

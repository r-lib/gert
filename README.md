...
However the problem is that configuring and building libgit2 is complicated (like most system libraries) and requires several platform-specific flags and system dependencies. As a result, git2r is sometimes installed with missing functionality, depending on what was detected during compilation. On MacOS for example, some git2r users have SSH support but others do not. Weird problems due to missing libgit2 features turn out to be very persistent, and have caused a lot of frustration. For this reason, gert does not bundle the libgit2 source, but instead always links to system libraries.

As usual, those who install gert as a source package, by choice on Windows and macOS or by necessity on Linux, do need to ensure the necessary system libraries are present: e.g., [libgit2-dev](https://packages.ubuntu.com/focal/libgit2-dev) on Debian/Ubuntu, [libgit2-devel](https://src.fedoraproject.org/rpms/libgit2) on Fedora, [Homebrew libgit2](https://github.com/Homebrew/homebrew-core/blob/master/Formula/libgit2.rb) on macOS, [rtools40 libgit2](https://github.com/r-windows/rtools-packages/blob/master/mingw-w64-libgit2/PKGBUILD) on Windows.
